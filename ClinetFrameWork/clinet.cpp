...
#include <WS2tcpip.h>
#define  STRLEN 1024
void prinerrorMsg_Cleint(const std::string& error) {
    std::cout << error;
}
...
int main222(int argc, char* argv[]) {
    ...
    char msg[STRLEN];
    if (argc != 3) {
        prinerrorMsg_Cleint(" parameter  number wrong\n");
        return 0;
    }
    if (WSAStartup(MAKEWORD(2, 2), &wsdata) != 0) {
        prinerrorMsg_Cleint("error wsadata");
        return 0;
    }
    hClntSock = socket(PF_INET, SOCK_STREAM, 0);// 申请一个套接字
    if (hClntSock == INVALID_SOCKET) {
        prinerrorMsg_Cleint("error hosck");
        return 0;
    }
   ...
   if (connect(hClntSock,(SOCKADDR*)&clnAddr,sizeof(clnAddr)) == SOCKET_ERROR) {
       prinerrorMsg_Cleint("connect error");
   }
   auto strlen =  recv(hClntSock, msg, sizeof(msg), 0);
   if (strlen == -1) {
       prinerrorMsg_Cleint("recv error");
   } else {
   std::cout<<msg;
   }
